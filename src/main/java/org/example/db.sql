...
-- Моделируем схему таблиц.
-- Кафе, блюда, заказы
-- Цель - заказ еды в каком-нибудь кафе.
-- Каждая команда в своих запросах может использовать только следующие join. Указан PK, FK, который обязательно надо использовать, другие использовать не запрещается.
-- Left join
-- Right join
-- Full join
-- Inner join
-- Сделать запросы. Которые позволяют увидеть:
